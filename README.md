...
도시 교차로 다수는 여전히 **고정형·시간대(TOD) 신호 운영**에 의존해 실시간 교통 변화에 능동적으로 대응하기 어렵고, 공공·민간 교통 데이터도 곳곳에 흩어져 있습니다. **Syncro**는 V2X·TOPIS 등 **19개 공공/민간 API**를 실시간으로 통합하고, 외부 의존 없는 **로컬 LLM**이 교차로 혼잡을 분석해 신호 조정안을 권고하여 **데이터 기반 교통 관제**를 실현합니다. (서울시 25개 구 대상)
...
| 통합 대시보드 | 실시간 지도 |
|:---:|:---:|
| <img src="docs/images/01-dashboard.png" width="100%"/> | <img src="docs/images/02-map.png" width="100%"/> |
| 위험 교차로·구별 현황·교통량 예측을 한 화면에 | 교차로·신호·위험도·민원·CCTV 통합 조회 |

| 신호 시뮬레이션 (3D) | 교통 뉴스 분석 |
|:---:|:---:|
| <img src="docs/images/03-simulation.png" width="100%"/> | <img src="docs/images/05-news.png" width="100%"/> |
| VWorld 3D 주행뷰 · 신호 조정 전후 비교 | NAVER 뉴스 크롤링 · BERT 유형 분류 · 과장도 분석 |

| CCTV 모니터링 | 민원 접수 |
|:---:|:---:|
| <img src="docs/images/04-cctv.png" width="100%"/> | <img src="docs/images/06-complaint.png" width="100%"/> |
| UTIC 실시간 CCTV 302개 스트림 | AI 이미지 분류 · 부서 자동 배정 |
...
**TEAM FINAL** — 산업공학 · AI · 풀스택을 결합한 4인 팀 · **개발 기간 2025.05.04 ~ 2025.06 (약 6주, 메타빌드 최종 프로젝트)**

| 팀원 | 역할 | 주요 담당 |
|:---:|:---:|---|
| **박희윤** (팀장) | **AI 에이전트** | ReAct 에이전트 + MCP 도구 · 멀티에이전트 신호 최적화 · 실시간 수집 파이프라인 · 캐싱·WebSocket 브로드캐스트 · 뉴스 파이프라인 |
| **김수아** | **프론트엔드 · 시각화** | 화면 구현·데이터 시각화 · 지도 기반 교차로 선택/상세 · VWorld 3D 주행 뷰 · 신호 운영계획 모델링·DB |
| **설석현** | **백엔드 · 데이터 · API** | 데이터 가공·통합(TrafficStatus) · 공공 API 연동(TOPIS·구간속도·기상청) · 위험도/교통량 · 예방 알고리즘·단위테스트 |
| **진민경** | **교통량 예측 · 분석** | 예측 모델 개발·성능평가 · 예측 결과 프론트 연동·시각화 · 시뮬레이션 예측 연동·경로 분석 |

개발 단계: `계획(05.04~)` → `요구 분석(05.11~)` → `설계(05.18~)` → `구현(05.25~)` → `시험 & 피드백(06.12~)`

---

## 📜 라이선스

본 프로젝트는 **메타빌드 부트캠프 최종 팀 프로젝트**로 제작된 교육용 결과물입니다.
별도 표기가 없는 한 코드·디자인의 무단 상업적 사용 및 재배포를 제한합니다.
