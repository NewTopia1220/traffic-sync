...
function tryBuildMarkerGraphRoute(startCr, endCr, crossroads, totalDist, maxEdgeMeters) {
  ...
  const detourLimit = Math.max(totalDist * 10.0, totalDist + 4000);
  ...
  const middleNodes = crossroads
    ...
    .map(cr => {
      ...
      const score =
        ...
        Math.max(0, detour - totalDist) * 0.15 +  // detour 위주로
        projected.distance * 0.20 +               // 0.58 → 0.28로 낮춤
        Math.abs(projected.progress - 0.5) * 10;  // 35 → 20으로 낮춤
...
export default function SimulationMapView({ selectedList = [], onSelect, isOptimized = false, onStatsChange, onAutoWaypointsChange }) {
  ...
  useEffect(() => {
    ...
    routePoints.map(p => `${p.lon},${p.lat}`).join("|"),
    isOptimized,
    cesiumReady
  ]);
  ...
  function flyToSelectedArea() {
    ...
    // 목적지를 클릭한 직후에는 routePlan이 비동기로 계산되기 전이라
    // routePoints가 잠깐 빈 배열일 수 있습니다.
    // 이 상태에서 mid.lon을 읽으면 화면이 터지므로, 경로가 준비된 경우에만
    // 경로 중심으로 이동하고 아니면 출발지 기준으로 안전하게 이동합니다.
    if (endLL && routePoints.length >= 2) {
      const mid = interpolateRoute(routePoints, 0.5);
      const dist = routeLengthMeters(routePoints);
      if (mid && Number.isFinite(mid.lon) && Number.isFinite(mid.lat) && dist > 0) {
