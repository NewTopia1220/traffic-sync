...
export function createSignalCanvas(isRed) {
  ...
  ctx.fillStyle = isRed ? "rgba(127,29,29,0.85)" : "rgba(22,101,52,0.85)";
  ctx.fill();
  ctx.shadowBlur = 0;
  ctx.lineWidth = 4;
  ctx.strokeStyle = color;
...
export function createDirectionalSpeedLabelCanvas(directionLabel, speedKph, congestion, axisName = "") {
  ...
  if (isBottleneck) {
    ...
    ctx.fillText(roadText ? `${directionLabel} · ${roadText}` : directionLabel, canvas.width / 2, 105);
    return canvas.toDataURL();
  }
...
export function distanceToPolylineMeters(point, vertices) {
  ...
  for (let i = 0; i < vertices.length - 1; i++) {
    ...
    minDistance = Math.min(minDistance, Math.hypot(p.x - (a.x + dx * t), p.y - (a.y + dy * t)));
  }
  return minDistance;
}
...
export function mapCarToTrafficDirection(carPos, segment) {
  ...
  return { direction, traffic: segment[direction], distanceMeters: direction === "up" ? upDistance : downDistance, upDistanceMeters: upDistance, downDistanceMeters: downDistance };
}
