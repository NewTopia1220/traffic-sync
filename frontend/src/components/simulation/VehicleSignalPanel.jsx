...
export default function VehicleSignalPanel({
  ...
  if (loading) return <div style={{ padding: "12px 0", fontSize: 12, color: "#64748b", textAlign: "center" }}>신호 데이터 로딩 중...</div>;
  if (!phases.length) return <div style={{ padding: "22px 10px", color: "#64748b", fontSize: 13, lineHeight: 1.7, textAlign: "center" }}>신호계획 데이터가 비어 있습니다.</div>;
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
        {phases.map(phase => {
          ...
          const borderColor = isVehicleFollowing
            ...
            : isCurrent ? "rgba(34,197,94,0.45)" : "rgba(255,255,255,0.06)";
          const bgColor = isVehicleFollowing
            ? isVehicleStopPhase ? "rgba(127,29,29,0.26)" : "rgba(22,101,52,0.25)"
            : isCurrent ? "rgba(22,101,52,0.18)" : "rgba(255,255,255,0.025)";
          ...
          return (
            <div key={phase.no} style={{
              ...
              <span style={{ color: isVehicleFollowing ? "#ffffff" : isCurrent ? "#22c55e" : "#94a3b8", fontSize: 12, fontWeight: isVehicleFollowing || isCurrent ? 900 : 700, minWidth: 42 }}>
                현시 {phase.no}
              </span>
              ...
              <span style={{ marginLeft: "auto", color: "#64748b", fontSize: 11, fontFamily: "monospace" }}>{phase.sec}s</span>
            </div>
          );
        })}
      </div>
